...
// Functions
static bool ImGui_ImplWin32_InitEx(void* hwnd, bool platform_has_own_dc, void* glcontext = nullptr)
{
	...
	if (glcontext) bd->glContext = (HGLRC)glcontext;
...
IMGUI_IMPL_API bool     ImGui_ImplWin32_InitForOpenGL(void* hwnd, void* glcontext)
{
    ...
    return ImGui_ImplWin32_InitEx(hwnd, true, glcontext);
}
...
struct ImGui_ImplWin32_ViewportData
{
    ...
    ImGui_ImplWin32_ViewportData() { Hwnd = HwndParent = nullptr; HwndOwned = false;  DwStyle = DwExStyle = 0; Hdc = nullptr; HgLrc = nullptr; }
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
	...
	if (bd->glContext) {
		...
		int pixelFormat = ChoosePixelFormat(vd->Hdc, &pfd);
		SetPixelFormat(vd->Hdc, pixelFormat, &pfd);
		vd->HgLrc = wglCreateContext(vd->Hdc);
		wglMakeCurrent(vd->Hdc, vd->HgLrc);
		SwapBuffers(vd->Hdc);
		wglShareLists(bd->glContext, vd->HgLrc);
	}
}
...
static void ImGui_ImplWin32_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    if (vd->HgLrc == NULL) vd->HgLrc = wglCreateContext(vd->Hdc);

    ImDrawList* list = ImGui::GetBackgroundDrawList(viewport);

    bool isMadeCurrent = wglMakeCurrent(vd->Hdc, vd->HgLrc);
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle((void*)hWnd))
    {
        switch (msg)
        {
		// Not needed, due to OWN_DC Feature
        //case WM_ERASEBKGND:
        //    return 0;
        case WM_CLOSE:
